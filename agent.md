# agent.md — WorkBuddy × Hermes 协作规范（v1.1，2026-08-11）
...
### 3.3 轮次计数与 Open Issues 登记（v1.1 新增）
...
- 后续任务交接时随附未关闭的 Open Issues 清单，确保遗留项不丢失；遗留项关闭需对方确认。
...
| `协同工作文档/` | 交接记录、审查报告 | **子目录 `审查报告/`**，命名 `YYYY-MM-DD-审查-第N轮.md`；遗留项登记 `Open-Issues.md` |
