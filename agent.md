# agent.md — Hermes × zcode × codex × qoder 协作规范（v1.8，2026-08-23）
...
> **本文件是 Hermes、zcode、codex、qoder、WorkBuddy 各方在本项目中的协作运行时规范。各方每次开工前必须读取本文件。**
...
| **qoder（2026-08-23 Vange 加入）** | **职责待定（先用着）**：方向未定，后续 Vange 指定；产出文件后缀 `_qd` | 待定（Vange 后续指定） | — |
...
| `_qd` | qoder（2026-08-23 Vange 加入） | **职责待定**（先用着，方向后续 Vange 指定） |
...
- **v1.6**（2026-08-19）：Vange 判定——WorkBuddy 暂停（完成 CosyVoice 部署收尾后停止一切事务），开发职责全权移交 zcode；原「动工前核实 Kimi K3」条款废止。详见 `协同工作文档/通知与对齐/2026-08-19-角色变更通知-WB暂停-zcode接管_hm.md`。
...
- **v1.9**（2026-08-23）：新增角色 **qoder**（Vange 加入）——**职责待定（先用着）**，方向后续 Vange 指定；文件后缀 `_qd`。协作方更新为 Hermes × zcode × codex × qoder（+ WorkBuddy 暂停保留）。
