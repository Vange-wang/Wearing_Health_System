...
10. **重搏切迹伪峰（2026-08-23 微动修复）**：6mA 高信噪比下 PPG 重搏切迹（主峰后 300-400ms 次峰）被峰检测当作独立心跳，HR 在真实值与 2 倍值间二选一跳变（65 vs 172 均"规整"）。修复：峰最小间距 0.3s→0.45s（心率上限 133 BPM）+ LED 电流回退 4.6mA 减弱切迹
11. **HR 峰检测微动敏感（2026-08-23 修复）**：三重门限——qcd 四分位离散度 ≤0.50（静置 0.18-0.53 / 运动 0.56-2.8 实测标定）+ 变化率门限（相对稳定值跳变 >30% 需 qcd≤0.10 极规整）+ 冷启动连续 2 个一致窗口（±20 bpm）才锁定；运动期维持上一稳定值 + flags bit2 伪影位；输出 3 帧中位数平滑；SpO2 与 HR 同门控（运动期 R 比率失真假低）。最终与血氧仪对照偏差 -3 bpm
