...
|---|---|---|---|---|---|---|
| OI-001 | health tts 字段只返回主引擎名（如 "edge"），edge 不可用时仍显示 "edge"，未反映实际可用引擎。建议 v0.2 改进为显示实际可用引擎（如 "piper(fallback)"）或连通性检查 | NON_SERIOUS | **closed** | WorkBuddy | 2026-08-11 | 2026-08-15 |
...
| OI-004 | edge-tts 主引擎不可用（微软封 6.x token, HTTP 403），v0.1 实际 piper 独撑 TTS。v0.2 需评估恢复方案：升 edge-tts 7.x+音频转码（需引入 ffmpeg/pydub，评估是否违反依赖红线）或替换为火山/讯飞云 TTS | NON_SERIOUS | **closed** | Hermes | 2026-08-11 | 2026-08-15 |
...
| OI-007 | SenseVoice 对纯静音/低能量音频会幻觉出噪声字符（如韩文字符），当前通过 0 帧 WAV 触发 no_speech 兜底。v0.2 建议加 VAD 或能量门限做前置过滤 | NON_SERIOUS | **closed** | Hermes | 2026-08-11 | 2026-08-15 |

---

*最后更新：2026-08-15 · v0.2 REVIEW_PASS 后：OI-001/004/007 经 Hermes 验证确认关闭，全部 7 条 OI 已 closed · 关闭依据：2026-08-15-v0.2自测报告-终审报告.md（OI-001/007 实现验证）、规划文档/技术验证/2026-08-15-edge-tts恢复评估-OI004.md（OI-004 评估完成）*
