...
# 问候/礼貌/结束语白名单：这些词不是追问，直接走快路径
# （否则 DATA/慢路径后说「你好」「谢谢」会被错误继承为上一轮路由）。
POLITE_WHITELIST = [
    "你好", "您好", "谢谢", "再见", "晚安", "早安", "拜拜",
    "辛苦", "不用", "没事", "好的", "嗯嗯", "收到", "好嘞",
]
...
# 健康领域词用于把非确定性报数问题留在轻量/RAG 通道，避免被通用工具词截走。
HEALTH_DOMAIN_TERMS = ("心率", "心跳", "血氧", "血压")

# 命中这些知识/判断提示词时，即使句中同时出现 DATA 查询短语，也不得退化为模板报数。
...
class Router:
    ...
    def route(self, text: str, rag_hit: bool = False) -> str:
        ...
        1. 命中无知识意图的当前健康数值短语 → DATA（模板直答）
        2. 命中知识库索引（rag_hit=True）→ RAG
        3. 其余健康领域问题 → LIGHTWEIGHT
        4. 上轮慢路径（未过 TTL）+ 本轮指代追问 → 延续上一 DATA/HERMES 路由
        5. 命中技能/工具需求关键词 → HERMES（慢路径，先安抚）
        6. 其余 → LIGHTWEIGHT
        ...
        if self._is_capability_ask(text):
            ...
            return LIGHTWEIGHT
        # 问候/礼貌/结束语（你好/谢谢/再见/晚安…）→ 快路径，不延续慢路径
        if self._is_polite(text):
            logger.info("route=lightweight (polite: %s)", text)
            self._set_last_route(LIGHTWEIGHT)
            return LIGHTWEIGHT
        # DATA 只接收无知识意图的窄查询短语；知识/关系/方法问题不得被字段子串截胡。
        if self._is_data_query(text):
            logger.info("route=data (current health value query)")
        ...
        if rag_hit:
            ...
            return RAG
        # 未命中知识库的健康领域问题留在 lightweight。该分支必须早于通用工具词，
        # 否则「帮我看看心率」会被「看看」误送到 Hermes 慢路径。
        if self._is_health_domain(text):
            logger.info("route=lightweight (health domain)")
            self._set_last_route(LIGHTWEIGHT)
            return LIGHTWEIGHT
    ...
    def _is_polite(self, text: str) -> bool:
        """问候/礼貌/结束语（你好/谢谢/再见/晚安…）：不是追问，直接快路径。"""
